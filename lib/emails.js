 ...
 * @param {{ downloadPdfUrl?: string, multiPdfAttachments?: Array<{ filename: string, content: string }> }} [options] — lien signé pour le bouton principal ; ou plusieurs PDF (commande pack)
...
export async function sendDeliveryEmail(email, prenom, pdfBuffer, audioBuffer, options = {}) {
  ...
  const multi = Array.isArray(options.multiPdfAttachments) && options.multiPdfAttachments.length > 0;

  let attachments;
  let hasAudio = false;

  if (multi) {
    attachments = options.multiPdfAttachments.map((a) => ({
      filename: a.filename,
      content: typeof a.content === "string" ? a.content : toBase64Attachment(a.content),
    }));
  } else {
    const safeFile =
  ...
  const subject = multi
    ? `🌙 Tes histoires Qissali sont prêtes (${options.multiPdfAttachments.length}) !`
    : `🌙 L'histoire de ${prenomLabel} est prête !`;
